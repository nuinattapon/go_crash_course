...
func Prepare(ctx context.Context, db DBTX) (*Queries, error) {
	...
	if q.lastInsertIdStmt, err = db.PrepareContext(ctx, lastInsertId); err != nil {
		return nil, fmt.Errorf("error preparing query LastInsertId: %w", err)
	}
	if q.listAuthorIDStmt, err = db.PrepareContext(ctx, listAuthorID); err != nil {
		return nil, fmt.Errorf("error preparing query ListAuthorID: %w", err)
	}
	...
	if q.rowsAffectedStmt, err = db.PrepareContext(ctx, rowsAffected); err != nil {
		return nil, fmt.Errorf("error preparing query RowsAffected: %w", err)
	}
...
func (q *Queries) Close() error {
	...
	if q.lastInsertIdStmt != nil {
		if cerr := q.lastInsertIdStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing lastInsertIdStmt: %w", cerr)
		}
	}
	if q.listAuthorIDStmt != nil {
		if cerr := q.listAuthorIDStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing listAuthorIDStmt: %w", cerr)
		}
	}
	...
	if q.rowsAffectedStmt != nil {
		if cerr := q.rowsAffectedStmt.Close(); cerr != nil {
			err = fmt.Errorf("error closing rowsAffectedStmt: %w", cerr)
		}
	}
...
type Queries struct {
	...
	lastInsertIdStmt *sql.Stmt
	listAuthorIDStmt *sql.Stmt
	listAuthorsStmt  *sql.Stmt
	rowsAffectedStmt *sql.Stmt
...
func (q *Queries) WithTx(tx *sql.Tx) *Queries {
	return &Queries{
		...
		lastInsertIdStmt: q.lastInsertIdStmt,
		listAuthorIDStmt: q.listAuthorIDStmt,
		listAuthorsStmt:  q.listAuthorsStmt,
		rowsAffectedStmt: q.rowsAffectedStmt,
